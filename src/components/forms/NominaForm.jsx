...
export default function NominaForm({ initial, personal, onSave, onClose }) {
  ...
  const [f, setF] = useState(
    initial || {
      personalId: empleados[0]?.id || '', periodoInicio: '', periodoFin: todayISO(), liquidado: '', cotizacionSs: '',
  ...
  return (
    <Modal title={initial && initial.id ? 'Editar nómina' : 'Nueva nómina'} onClose={onClose}>
      ...
      <div className="grid2">
        ...
        <Field label="Fecha de pago"><input type="date" value={f.fechaPago || ''} onChange={(e) => set('fechaPago', e.target.value)} /></Field>
      </div>
