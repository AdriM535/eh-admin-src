...
export default function Nominas({ data, actions, calc, setModal }) {
  ...
  const nominas = data.nominas
    ...
    .filter((n) => tipoFilter === 'todos' || n.tipo === tipoFilter)
    .slice()
    .sort((a, b) => (b.periodoFin || '').localeCompare(a.periodoFin || ''));
  const total = nominas.reduce((s, n) => s + Number(n.total || 0), 0);
  const totalBonos = data.nominas.filter((n) => n.tipo === 'bono_extra').reduce((s, n) => s + Number(n.total || 0), 0);
  ...
  return (
    <>
      <div className="pagehead">
        <div>
          <h1>Nóminas</h1>
          <div className="desc">Liquidaciones periódicas y bonos/pagos extra al personal empleado</div>
        </div>
      ...
      <div className="ledger" style={{ gridTemplateColumns: 'repeat(3,1fr)' }}>
        ...
        <div className="cell"><div className="lbl">Bonos/extra fuera de nómina (total)</div><div className="val">{fmtMoney(totalBonos)}</div></div>
      </div>
      <div className="toolbar">
        <div className="filters">
          ...
          <select value={tipoFilter} onChange={(e) => setTipoFilter(e.target.value)}>
            <option value="todos">Nóminas y bonos</option>
      ...
      <div className="tblwrap">
        <table>
          <thead><tr><th>Trabajador/a</th><th>Tipo</th><th>Periodo</th><th>Total</th><th>Estado</th><th></th></tr></thead>
          <tbody>
            {nominas.length === 0 && <tr><td colSpan="6" className="empty">Sin nóminas registradas.</td></tr>}
            {nominas.map((n) => {
              const p = calc.personalById(n.personalId);
              return (
                <tr key={n.id}>
                  <td>{p ? p.nombre : '—'}</td>
                  <td>{n.tipo === 'bono_extra' ? <span className="pill ochre">Bono / extra</span> : <span className="pill">Periódica</span>}</td>
