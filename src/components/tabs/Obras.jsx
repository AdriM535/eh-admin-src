...
export default function Obras({ data, actions, calc, setModal }) {
  ...
  return (
    <>
      ...
      {obras.length === 0 && <div className="empty">Sin obras registradas.</div>}
      ...
      <div className="cards" style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(340px,1fr))' }}>
        {obras.map((o) => {
          ...
          return (
            <div className="ficha" key={o.id}>
              {estadoPill(o.estado)}
              <h3 style={{ marginTop: 14 }}>{o.nombre}</h3>
              <div className="meta">
                <b>Cliente:</b> {cli ? cli.nombre : 'sin asignar'}<br />
                {o.direccion && <>{o.direccion}<br /></>}
                {o.fechaInicio && <>Inicio: {fmtDate(o.fechaInicio)} </>}
                {o.fechaFin && <>· Fin: {fmtDate(o.fechaFin)}</>}
              </div>
              <div style={{ display: 'flex', gap: 14, marginTop: 10, fontSize: 12.5 }}>
                <div><div style={{ color: 'var(--ink-soft)' }}>Facturado</div><b>{fmtMoney(o.stats.totalFacturado)}</b></div>
                <div><div style={{ color: 'var(--ink-soft)' }}>Gastos</div><b>{fmtMoney(o.stats.totalGastos)}</b></div>
                <div><div style={{ color: 'var(--ink-soft)' }}>Margen</div><b style={{ color: o.stats.margen >= 0 ? 'var(--green)' : 'var(--brick)' }}>{fmtMoney(o.stats.margen)}</b></div>
              </div>
              {o.stats.pendienteCobro > 0 && <div style={{ marginTop: 6 }}><span className="pill brick">Pendiente de cobro: {fmtMoney(o.stats.pendienteCobro)}</span></div>}
              <div className="rowbtns">
                <button className="btn ghost small" onClick={() => setExpanded(open ? null : o.id)}>{open ? 'Ocultar detalle' : 'Ver detalle'}</button>
              ...
              {open && (
                <div style={{ marginTop: 12, borderTop: '1px solid var(--line-soft)', paddingTop: 10, fontSize: 12.5 }}>
                  ...
                  {o.stats.compras.map((c) => (
                    <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0' }}>
                      <span>{c.proveedor || c.tipoGasto} — {fmtDate(c.fecha)}</span>
